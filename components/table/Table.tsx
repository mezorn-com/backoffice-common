...
const Table = ({
	...
	React.useEffect(() => {
		const ids = table
			.getSelectedRowModel()
			.rows.map(row => row.original._id) as string[];
	...
	}, [ table.getSelectedRowModel().rows, externalState.selectedRows ]);
				...
				<Menu shadow='md' width={200}>
					...
					<Menu.Dropdown>
						{Object.keys(externalState.bulkItemActions).map(key => {
							const bulkAction =
								externalState.bulkItemActions?.[key];
							if (!bulkAction) {
								return null;
							}
							return (
